 ...
 *
 * On Vercel without DATABASE_URL we stay "pglite" in name only — callers should
 * not boot WASM Postgres in a serverless isolate.
 */
...
export async function getPglite(): Promise<import("@electric-sql/pglite").PGlite> {
  if (dbSource !== "pglite" || onServerless) {
...
export function ensureDbReady(): Promise<void> {
  if (dbSource !== "pglite" || onServerless) return Promise.resolve();
...
if (typeof window === "undefined" && dbSource === "pglite" && !onServerless) {
